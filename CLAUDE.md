...
- 15-phase roadmap with goals + success criteria + cumulative reqs (Phase 3 REDIRECTED 2026-05-04 → Phase 3.1 + 3.2 + 3.3 + 3.4 inserted; architecture revised → NeoCadeTheme superclass carries chosen base direction's style + per-theme subclasses override deltas via `super._regenerate()` pattern; Phase 3.3 derives 5 candidate directions from user goals + restrictions + v0 reactions as DNA — direction names kept/revised/replaced per fit, only Boardwalk Sunset hard-rejected; base-direction picked by user at Phase 3.4 gate): [.planning/ROADMAP.md](.planning/ROADMAP.md)
...
- **MD3 + MD3 Expressive design language** (created by Phase 3.1; consumed by Phase 3.3 + 3.4 + 4-7): `.planning/research/MD3-RESEARCH.md` (does not exist until Phase 3.1 closes)
- **Flat-3D Game UI / Extruded Flat UI patterns** + Godot StyleBoxFlat translation recipes (created by Phase 3.1; consumed by Phase 3.4 raised variation + Phase 4-7): `.planning/research/FLAT-3D-UI-RESEARCH.md` (does not exist until Phase 3.1 closes)
- **Godot Dynamic Theme Architecture** + feasibility validation (created by Phase 3.2; consumed by Phase 3.4 + Phase 4 NeoCadeTheme implementation): `.planning/research/GODOT-DYNAMIC-THEME-RESEARCH.md` + `.planning/spikes/dynamic-theme/` (do not exist until Phase 3.2 closes)
- **5 user-approved candidate theme directions** with names + base/accent colors + personalities (created by Phase 3.3; consumed by Phase 3.4 mockups + Phase 4 subclass implementation): `.planning/research/THEME-DIRECTIONS.md` (does not exist until Phase 3.3 closes; **direction names: keep, revise, or replace v0 names per fit** — only Boardwalk Sunset is hard-rejected; ALL directions get universal anti-texture/flat-MD3 revisions regardless of name retention; **base-direction designation deferred to Phase 3.4** user-approval gate where one of the N approved directions is picked as `NeoCadeTheme`'s default style)
- **Phase 3 v0 redirect notes** + user feedback log per direction (MUST READ when working on Phase 3.3 — it's the DNA input map): [.planning/phases/03-visual-direction-mockup-approval-gate/REDIRECTED.md](.planning/phases/03-visual-direction-mockup-approval-gate/REDIRECTED.md)
...
**Refuse:** parallel `/gsd-*` commands (state collisions); hand-editing `.planning/*` mid-execution
