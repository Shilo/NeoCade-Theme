...
**Core deliverable:** `res://addons/neocade_theme/themes/{theme}_{flat|raised}_{desktop|mobile}.tres` for every (theme × variation × platform) combination user approves at the Phase 3.2 gate. Each `.tres` must be feature-complete to `godot-minimal-theme`'s bar — every one of the 35 user-facing Godot 4.6 Control classes themed across every state. v1 file count is `4N` where N = approved theme count (typical 1-3 for v1).
...
- 13-phase roadmap with goals + success criteria + cumulative reqs (Phase 3 REDIRECTED 2026-05-04 → Phase 3.1 + 3.2 inserted): [.planning/ROADMAP.md](.planning/ROADMAP.md)
...
- **MD3 + MD3 Expressive design language** (created by Phase 3.1; consumed by Phase 3.2 mockups + Phase 4-7): `.planning/research/MD3-RESEARCH.md` (does not exist until Phase 3.1 closes)
- **Flat-3D Game UI / Extruded Flat UI patterns** + Godot StyleBoxFlat translation recipes (created by Phase 3.1; consumed by Phase 3.2 raised variation + Phase 4-7): `.planning/research/FLAT-3D-UI-RESEARCH.md` (does not exist until Phase 3.1 closes)
- **Phase 3 v0 redirect notes** + user feedback log per direction (MUST READ when working on Phase 3.2 or revisiting any v0 direction): [.planning/phases/03-visual-direction-mockup-approval-gate/REDIRECTED.md](.planning/phases/03-visual-direction-mockup-approval-gate/REDIRECTED.md)
...
- Final design tokens (specific hex / px values + token-matrix structure for N themes × 4 variations; **created by Phase 3.2** after mockup approval; consumed by Phases 4-9): `.planning/DESIGN_TOKENS.md` (does not exist until Phase 3.2 closes)
