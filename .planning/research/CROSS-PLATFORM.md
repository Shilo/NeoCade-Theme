...
| Variable font `wght`/`opsz` axes | Supported. Inter Variable + Noto Sans Variable + Outfit Variable all render correctly. | LOW | Godot's FontVariation is engine-side — works wherever FontFile loads. |
| App Store font licensing | **OFL 1.1 and Apache 2.0 both PASS App Store review.** Inter (OFL 1.1), Noto Sans (OFL 1.1), Outfit (OFL 1.1), JetBrains Mono (OFL 1.1) all permit "embedded in commercial software including App Store apps" per OFL FAQ. | LOW | [SIL OFL FAQ](https://openfontlicense.org/ofl-faq/), [Inter LICENSE.txt](https://github.com/rsms/inter/blob/master/LICENSE.txt), [openfontlicense.org/how-to-use-ofl-fonts](https://openfontlicense.org/how-to-use-ofl-fonts/). |
