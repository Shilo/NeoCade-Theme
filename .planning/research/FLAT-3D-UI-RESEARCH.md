...
Raised behavior categories used below: `changes strongly`, `changes subtly`, `normally stays flat`, `no visible raised delta`, `NeoCade-owned decision`.

| Family | Controls covered | `raised=false` visual behavior | `raised=true` visual behavior | Category | Rationale | Evidence basis |
|---|---|---|---|---|---|---|
| Buttons and button-like controls | Button, PrimaryButton, SecondaryButton, DangerButton, GhostButton, IconButton, OptionButton, MenuButton, ColorPickerButton, LinkButton where applicable | Flat MD3 button fill/outline/text state model. | Solid top shape plus offset darker duplicate for filled/high-value buttons; outlined/text buttons may remain flat or get subtle pressed depth only. | changes strongly | Buttons are the clearest pressable affordance and best match the user exemplars. | User exemplars + Flat-3D principle + MD3 button hierarchy. |
| Text inputs and editors | LineEdit, TextEdit, CodeEdit, SpinBox | Flat/outlined text-field surface with focus ring, caret, selection, placeholder, gutter where needed. | Normally stays flat; SpinBox steppers may show subtle button-like treatment, but the input field should not lift. | normally stays flat | Inputs need readability and stable text baselines more than tactile depth. | MD3 text-field principle; NeoCade-owned decision for CodeEdit/SpinBox composition. |
| Panels and containers | Panel, PanelContainer, MarginContainer, ScrollContainer, FoldableContainer body, layout containers | Tonal surface ladder, border/radius, no shadow. | Mostly stays flat; FoldableContainer header may gain subtle action treatment, but large content panels should not float. | normally stays flat | Raising large panels makes the UI decorative and risks fake 3D. | MD3 surface principle + Flat-3D hard-filter audit. |
| Popups, dialogs, windows, and tooltips | PopupMenu, PopupPanel, TooltipPanel, TooltipLabel, Window, AcceptDialog, ConfirmationDialog, FileDialog | Highest surface role, solid border/radius, clear typography/actions. | Selectively raised actions or title/action bars; whole popup/dialog remains tonal, not shadowed. | changes subtly | Dialogs need hierarchy, but large shell shadows conflict with no-soft-shadow rule. | MD3 dialog/menu principle + popup Window pitfall + NeoCade-owned decision. |
| Lists, trees, tabs, and menus | ItemList, Tree, TabBar, TabContainer, PopupMenu, MenuBar | Flat rows with hover/selected/focus state layers; icons/disclosure controls are crisp. | Selected tabs/chips or menu items may get subtle offset; rows normally stay flat. | changes subtly | Dense scan surfaces must stay stable; selected/actionable affordances can pop a little. | MD3 list/tab/menu principles + Flat-3D pattern catalogue. |
| Range controls and progress | HSlider, VSlider, HScrollBar, VScrollBar, ProgressBar | Flat track/fill/handle, focus ring and dragged 16% state. | Handles/grabbers may gain subtle hard offset while dragged/interactive; tracks and ProgressBar fill stay flat. | changes subtly | Handles are interactive; tracks are status surfaces. | MD3 state-layer values + NeoCade-owned decision. |
| Advanced controls: ColorPicker, GraphEdit, GraphNode, GraphFrame, FileDialog, FoldableContainer | ColorPicker, GraphEdit, GraphNode, GraphFrame, FileDialog, FoldableContainer | Composite surfaces follow their child families; graph canvas low-emphasis, nodes card-like, frames grouping-only. | GraphNode selection may gain stronger outline/offset if approved; GraphFrame normally flat; ColorPicker/FileDialog action buttons follow button rules. | NeoCade-owned decision | Public Flat-3D evidence is absent for these Godot-specific controls, so behavior derives from MD3/Flat-3D principles. | NeoCade-owned decision with source agreement from MD3 surfaces and Flat-3D affordance rules. |
| Passive text/decorative controls: Label, RichTextLabel, separators | Label, RichTextLabel, TooltipLabel text, HSeparator, VSeparator, non-interactive icon/text slots | Typography/color only; separators use outline/subtle line. | No visible raised delta. | no visible raised delta | Passive content should not imply pressability. | MD3 typography/divider principles + NeoCade-owned decision. |

Matrix rule: every `NeoCade-owned decision` row still chooses a category and rationale. Lack of public Flat-3D evidence for advanced Godot Controls is not permission to skip the matrix.
