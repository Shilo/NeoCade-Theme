...
| visual-identity-distinctiveness/002a | raised-depth-formula-current | comparison | Given a hot-pink `raised=true` button, when current `_raised_depth_color` renders, then depth strip drifts toward base+black | PENDING | identity, raised, comparison |
| visual-identity-distinctiveness/002b | raised-depth-formula-hsv-darken | comparison | Given the same button, when depth uses HSV value-darken with no base-pull, then depth stays in same hue family | PENDING | identity, raised, comparison |
| visual-identity-distinctiveness/003 | per-direction-signature-move-catalog | standard | Given the 5 directions, when each is forced to differ on at least one non-color/non-radius axis, then they no longer read as same theme + hex swap | PENDING | identity, direction |
