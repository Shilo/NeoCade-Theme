...
## PHASE6_RESOURCE_SAVER
##
## ResourceSaver round-trip helper for Phase 6. This is intentionally not run
## by Plan 06-01; Plan 06-05 runs it after all Phase 6 production bindings
## exist. The helper mirrors the Phase 5 data-only strip pattern:
##
...
func _init() -> void:
	print("PHASE6_RESOURCE_SAVER: starting round-trip for %d approved directions" % APPROVED_DIRECTIONS.size())
	var failures: Array[String] = []

	for path in APPROVED_DIRECTIONS:
		print("PHASE6_RESOURCE_SAVER: ---- %s ----" % path)
		...
		print("PHASE6_RESOURCE_SAVER_OK: %s (%d bytes, data-only)" % [path, size])

	if failures.is_empty():
		print("PHASE6_RESOURCE_SAVER OK: all direction resources round-tripped, stripped, and reloaded")
		quit(0)
		return
	push_error("PHASE6_RESOURCE_SAVER FAILED with %d failures" % failures.size())
	for failure in failures:
		push_error("  - %s" % failure)
		print("PHASE6_RESOURCE_SAVER_FAIL: %s" % failure)
	quit(1)
