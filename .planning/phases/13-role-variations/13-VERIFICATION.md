...
status: verified_docs_synced
score: 9/9 must-haves verified; manual visual halo check remains optional owner UAT
docs_synced: 2026-05-13
overrides_applied: 0
human_verification:
...
deferred_items:
  ...
  - id: WR-01 (from 13-REVIEW.md; closed by docs sync)
    location: ".planning/phases/13-role-variations/13-REVIEW.md"
    summary: "README wording now says Role Panels replace the panel face with a 6% role-color fill; the old compositing wording is historical."
    impact: "Closed documentation drift."
---
...
**Verified:** 2026-05-11
**Docs Sync:** 2026-05-13 — current live registry counts are `BINDING_TABLE.size() == 150` and `TYPE_VARIATIONS.size() == 62`; Phase 13 helper constants were updated to match. Historical command output below may still show the 2026-05-11 counts where it records the original run.
**Status:** verified_docs_synced (all 9 automated must-haves VERIFIED; visual UAT remains an optional owner check)
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 (SC-13-1) | Default chrome unchanged from Phase 12 baseline; smoke-test 30 configs regenerate cleanly with all invariants holding.                          | VERIFIED  | `_phase13_smoke_matrix.gd` PASS — 30 configs regenerated cleanly, all invariants held. Current docs-sync invariant counts are BT=150, TV=62, exports=12, with `has_color("font_color", *Label)` ×4 and `has_stylebox("panel", *Panel)` ×5. Default Label.font_color still uses `{"role": "text_strong"}`; default PanelContainer.panel still uses `"role": "surface_panel"` with `"alpha": "shape.surface_alpha_panels"` and `"padding": Vector2i(10, 8)`. |
...
| 6 | 9 Role Variation BINDING_TABLE recipe entries are present; current BINDING_TABLE.size() == 150.                                                          | VERIFIED  | Current live top-level key count is 150. 4 Role Label entries each contain only `color.font_color` recipe pointing to `role_<x>`. 5 Role Panel entries each contain a full `stylebox.panel` recipe with `alpha: 0.06` literal. |
...
| `.planning/phases/13-role-variations/helpers/_phase13_verify_headless.gd`                                          | Pin `EXPECTED_BINDING_TABLE_ROWS := 150` and `EXPECTED_TYPE_VARIATIONS_COUNT := 62`                                                       | VERIFIED   | Constants updated during the 2026-05-13 docs sync to match the current implementation. Historical runtime evidence from the original closeout remains below. |
| `.planning/phases/13-role-variations/helpers/_phase13_smoke_matrix.gd`                                             | Same pins (BT=150, TV=62)                                                                                                                 | VERIFIED   | Constants updated during the 2026-05-13 docs sync to match the current implementation. |
...
| ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------- |
| Architecture invariants (current: BT=150, TV=62, exports=12)             | `--stage architecture`                                                                   | Historical closeout output showed the then-current 149/61 counts; helpers now pin the 2026-05-13 live counts 150/62. | PASS     |
...
2. **WR-01** (from 13-REVIEW.md) — CLOSED by docs sync. README now describes Role Panels as replacing the panel face with a 6% role-color fill instead of compositing over the per-direction panel chrome.
