...
expected: Open `showcase/showcase.tscn` in Godot 4.6 (or run the project) and switch to the 10th "Role Variations" tab. Confirm:
  ...
  - The 5 Role Panels (`AccentPanel`, `InfoPanel`, `WarningPanel`, `DangerPanel`, `SuccessPanel`) render with a barely-visible translucent tint of their role color over the per-direction panel chrome — no harsh "stained-glass" overlay, no visible halo around panel borders, no banding artifacts under GL Compatibility.
