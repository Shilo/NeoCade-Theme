...
| 2 | Coverage delta document compares godot-minimal-theme's entries to FEATURES.md user-facing matrix; flags additive Controls and missing classes | VERIFIED (math corrected post-verification 2026-05-04) | `MINIMAL-THEME-COVERAGE-DELTA.md` contains the 37-row scorecard covering all state-rich Controls; 8 NeoCade-additives explicitly enumerated (CodeEdit, FoldableContainer, SpinBox, ColorPickerButton, LinkButton, FileDialog, ConfirmationDialog, TooltipLabel); sum invariant **corrected to** `direct (24) + subclass-only (1: Window) + bare-class-unthemed (2: MenuBar, Panel) + container-chrome (2: HSplit, VSplit) + NeoCade-additive (8) = 37 row entries`. The earlier `23 + 8 + 2 + 2 = 35` was off-by-one (Window double-handled) and the inherited "35" was FEATURES.md's executive-summary header which doesn't match its YES-row count — see COVERAGE-DELTA.md's "Scope reconciliation note" for the full correction. |
...
| `.planning/research/MINIMAL-THEME-COVERAGE-DELTA.md` | 37-row scorecard + 8 NeoCade-additives + FlatButton note + sum invariant + cross-link to ARCHITECTURE.md | VERIFIED (math + cross-link corrected post-verification) | Coverage Scorecard table (37 rows), NeoCade-Additives (8) detail table, FlatButton type-variation note as research-only, Numeric Summary with corrected sum invariant `24 + 1 + 2 + 2 + 8 = 37`. ARCHITECTURE.md cross-link added at top so designers know palette/typography/state-system live there. |
