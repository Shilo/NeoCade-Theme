...
must_haves:
  truths:
    ...
    - "If still not approved after three rounds, workflow escalates instead of writing final tokens"
...
<task type="auto">
  ...
  <action>
    ...
    If revision round would exceed three total rounds, stop and escalate with a summary of remaining concerns. Do not write `DESIGN_TOKENS.md`.
  </action>
  <verify>
    If approved, `03-APPROVAL-LOG.md` contains `APPROVED`. If revised, it contains `Revision round`.
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    Required sections:
    - Approval summary with approved direction, date, user approval quote, and mockup paths.
    ...
    - Typography: Inter Variable Roman only; M3-derived type scale; opsz/wght guidance; synthetic italic; non-Latin system fallback; consumer-supplied mono override.
  ...
  <verify>
    ...
    Select-String -Path .planning\DESIGN_TOKENS.md -Pattern 'Inter Variable Roman'
