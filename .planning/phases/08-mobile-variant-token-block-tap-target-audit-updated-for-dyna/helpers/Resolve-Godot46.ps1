<#
.SYNOPSIS
    Resolve a local Godot 4.6.x console executable for Phase 7 verification.

.DESCRIPTION
    Search-only resolver adapted from the Phase 6 helper pattern. It never
    downloads or installs Godot. On success it writes this Phase 7 helper
...
function Get-CandidatePaths {
    ...
    foreach ($phase6Path in @(
        '.planning/phases/06-lists-layout-range-tree-itemlist-tabs-containers-sliders-des/helpers/godot-cli-path.txt'
    )) {
        if (Test-Path -LiteralPath $phase6Path -PathType Leaf) {
            $items.Add((Get-Content -LiteralPath $phase6Path -Raw).Trim()) | Out-Null
        }
    }
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .planning/phases/07-dialogs-popups-advanced-window-popups-menubar-colorpicker-gr/helpers/Resolve-Godot46.ps1 -VerifyOnly
```
