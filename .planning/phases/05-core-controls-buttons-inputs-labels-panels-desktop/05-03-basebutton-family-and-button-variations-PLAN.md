...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 4: Primary and ghost strategy values are closed-enum values, and the distinct approved-direction strategy set is at least four values unless DESIGN_TOKENS explicitly shares one.
    - Test 5: DangerButton uses `role_danger` from Plan 05-02's role table and never falls back to `surface_panel` because of a missing semantic role.
  </behavior>
  <action>Add BINDING_TABLE rows for the six Button variations per D-01 and TYPEVAR-01. Recipes must use `shape.primary_radius`, `shape.primary_padding`, `shape.primary_strategy`, `shape.ghost_strategy`, `shape.raised_lifts.primary`, `shape.raised_lifts.secondary`, and `shape.raised_lifts.ghost` as appropriate. DangerButton must use `role_danger` added by Plan 05-02; do not add a local fallback and do not let missing-role resolution fall back to `surface_panel`. IconButton must remain compact with deterministic padding, reusing `tokens.tapPadding` or an explicitly added shape key that the verifier checks, while still using the official focus overlay. FlatButton is the runtime variation named by TYPEVAR-01, not the deferred editor-only FlatButton class. Use strategy helpers from Plan 02 rather than ad hoc per-direction branching in each row. Addresses review HIGH: DangerButton references an explicit semantic role created before this plan.</action>
  <verify>
    <automated>$phaseDir = '.planning/phases/05-core-controls-buttons-inputs-labels-panels-desktop'; $logDir = Join-Path $phaseDir 'logs'; New-Item -ItemType Directory -Force $logDir | Out-Null; $godot = (Get-Content (Join-Path $phaseDir 'helpers/godot-cli-path.txt') -Raw).Trim(); $buttonLog = Join-Path $logDir '05-03-buttons-task1.log'; & $godot --headless --path . --script (Join-Path $phaseDir 'helpers/_phase5_verify_headless.gd') -- --stage buttons *> $buttonLog; if ($LASTEXITCODE -ne 0) { Get-Content $buttonLog; throw 'buttons verifier failed' }; if (Select-String -Path $buttonLog -Pattern '^(ERROR|SCRIPT ERROR):' -Quiet) { Get-Content $buttonLog; throw 'buttons verifier log contains ERROR or SCRIPT ERROR' }</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <verify>
    <automated>$phaseDir = '.planning/phases/05-core-controls-buttons-inputs-labels-panels-desktop'; $logDir = Join-Path $phaseDir 'logs'; New-Item -ItemType Directory -Force $logDir | Out-Null; $godot = (Get-Content (Join-Path $phaseDir 'helpers/godot-cli-path.txt') -Raw).Trim(); $buttonLog = Join-Path $logDir '05-03-buttons-task2.log'; & $godot --headless --path . --script (Join-Path $phaseDir 'helpers/_phase5_verify_headless.gd') -- --stage buttons *> $buttonLog; if ($LASTEXITCODE -ne 0) { Get-Content $buttonLog; throw 'buttons verifier failed' }; if (Select-String -Path $buttonLog -Pattern '^(ERROR|SCRIPT ERROR):' -Quiet) { Get-Content $buttonLog; throw 'buttons verifier log contains ERROR or SCRIPT ERROR' }</automated>
  </verify>
...
<task type="auto">
  ...
  <action>Enable the focus assertion group for the button stage. `_phase5_focus_probe.gd` must test Pulse, Slate, Bubble, Daybreak, and Burst, force focus on representative Button, CheckBox, CheckButton, and OptionButton controls, then assert the generated `focus` stylebox is transparent-bg, accent-bordered, uses direction-specific `focus_thickness`, and uses direction-specific `shape.focus_offset`. These structural assertions are mandatory and must pass in all environments. Pixel rendering is an optional enhancement: attempt a small Viewport render for representative focused controls and pixel-check that the accent ring is visible outside the control bounds when rendering is available; if headless GL Compatibility cannot produce a meaningful image, log `PHASE5_FOCUS_RENDER_SKIPPED` and pass only if the structural focus assertions succeeded. Do not hard-block Phase 5 on unavailable headless pixel output; full tab-walk visual QA remains Phase 10.</action>
  <verify>
