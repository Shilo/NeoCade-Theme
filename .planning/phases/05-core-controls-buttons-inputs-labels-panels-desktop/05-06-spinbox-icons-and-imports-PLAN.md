...
<task type="auto" tdd="true">
  ...
  <verify>
    <automated>$phaseDir = '.planning/phases/05-core-controls-buttons-inputs-labels-panels-desktop'; $logDir = Join-Path $phaseDir 'logs'; New-Item -ItemType Directory -Force $logDir | Out-Null; $godot = (Get-Content (Join-Path $phaseDir 'helpers/godot-cli-path.txt') -Raw).Trim(); $importLog = Join-Path $logDir '05-06-import.log'; & $godot --headless --path . --import --quit-after 2 *> $importLog; if ($LASTEXITCODE -ne 0) { Get-Content $importLog; throw 'Godot import failed' }; if (Select-String -Path $importLog -Pattern '^(ERROR|SCRIPT ERROR):' -Quiet) { Get-Content $importLog; throw 'Godot import log contains ERROR or SCRIPT ERROR' }; Select-String -Path addons/neocade_theme/icons/spinbox_up.svg,addons/neocade_theme/icons/spinbox_down.svg -Pattern '#FFFFFF'; Select-String -Path addons/neocade_theme/icons/spinbox_up.svg.import,addons/neocade_theme/icons/spinbox_down.svg.import -Pattern 'svg/scale=2.0|mipmaps/generate=true|compress/mode=0|process/fix_alpha_border=true'</automated>
  </verify>
