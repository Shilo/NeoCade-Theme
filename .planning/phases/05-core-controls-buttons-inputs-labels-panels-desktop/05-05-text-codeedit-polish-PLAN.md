...
must_haves:
  truths:
    ...
    - "CodeEdit folded icon wiring uses the official Godot 4.6 `folded` icon slot verified by the helper."
  artifacts:
  ...
  key_links:
    - from: "addons/neocade_theme/neocade_theme.gd"
      ...
      pattern: "breakpoint_color|code_folding_color|bookmark_color|executing_line_color|line_length_guideline_color|line_number_color"
    - from: "addons/neocade_theme/neocade_theme.gd"
      to: "addons/neocade_theme/icons/code_folded.svg"
      via: "official CodeEdit icon recipe"
      pattern: "code_folded|folded"
---
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 2: InfoText still uses `normal_font` and `normal_font_size`, and still lacks the wrong `font_size` slot, after the text-final changes.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Test 1: CodeEdit has `breakpoint_color`, `code_folding_color`, `bookmark_color`, `executing_line_color`, `line_length_guideline_color`, and `line_number_color`.
    - Test 2: CodeEdit uses `code_folded.svg` only through the official Godot 4.6 `folded` icon slot, with `Theme.get_icon_list("CodeEdit").has("folded")` asserted by the verifier.
    - Test 3: Godot import and verifier logs contain no `ERROR:` or `SCRIPT ERROR:` lines.
  </behavior>
  <action>Author `code_folded.svg` following the Phase 4 icon contract: 32x32 reference, monochrome `#FFFFFF`, imported with `svg/scale=2.0`, mipmaps enabled, `compress/mode=0`, and `process/fix_alpha_border=true`. Add CodeEdit gutter color slots named by Godot 4.6 docs/research: `breakpoint_color`, `code_folding_color`, `bookmark_color`, `executing_line_color`, `line_length_guideline_color`, and `line_number_color`. Wire the folded icon to the official CodeEdit `folded` icon slot. Before binding, the verifier must introspect `Theme.get_icon_list("CodeEdit")` and assert `folded` exists; if Godot 4.6 introspection disagrees, abort and replan rather than silently selecting a different slot. Addresses review MEDIUM: folded slot is pinned and self-checked.</action>
  <verify>
  ...
  <done>CodeEdit gutter colors, `line_number_color`, and `folded` icon wiring pass the text-final verifier with clean Godot logs.</done>
</task>
