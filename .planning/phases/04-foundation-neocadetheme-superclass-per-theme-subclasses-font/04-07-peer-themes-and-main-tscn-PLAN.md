...
must_haves:
  truths:
    ...
    - "**Cross-AI Cycle 3 N4 fix (Fix A — textual post-process):** AFTER each peer `ResourceSaver.save()`, the helper invokes `_strip_theme_entries(path)` (the static helper authored in Plan 04-06). Each peer `.tres` ends up data-only: `[gd_resource ...]` header + 9 `@export` lines on `[resource]`, NO `[sub_resource]` blocks, NO `theme_data/` lines. SC#6 satisfied. Each peer `.tres` final on-disk size MUST be < 2048 bytes (2 KiB)."
...
<task type="auto">
  ...
  <action>
    ...
    **Stage A — Extend `_phase4_import.gd`:**
    ...
    ## Cross-AI Cycle 3 N4 Fix A: after each save, _strip_theme_entries(path) is called
    ## (the static helper authored in Plan 04-06) to keep the file data-only per SC#6.
...
<task type="auto">
  ...
  <action>
    ...
    - N4 fix (Cycle 3): _save_peer_tres() body now invokes _strip_theme_entries(path)
      AFTER each ResourceSaver.save(t, path) — re-uses the static helper
      authored in Plan 04-06 Task 1. Each peer .tres ends data-only:
      [gd_resource ...] header + 9 @export lines, NO [sub_resource] blocks,
      NO theme_data/ lines. Per-peer post-strip size asserted < 2048 bytes
      (SC#6 satisfied).
