...
provides:
  ...
  - "main.tscn theme override restored — points at pulse_neocade_theme.tres (recommended starter)"
...
key-decisions:
  ...
  - "main.tscn theme reference uses type=\"Theme\" (max compatibility) rather than type=\"NeoCadeTheme\". Both forms are accepted by Godot 4.6 (NeoCadeTheme extends Theme); the resource resolves to its actual subclass at runtime regardless of the declared type tag in the .tscn. The plan explicitly authorized either form."
...
# Phase 4 Plan 07: Peer Themes (Slate/Bubble/Daybreak/Burst) + main.tscn Reassignment Summary

**Ship the four peer direction NeoCadeTheme .tres files (Slate/Bubble/Daybreak/Burst) per DESIGN_TOKENS §5.2-§5.5, extend the build-time generator with _save_peer_tres(), close the Cycle 2 M3 peer-validation gap by extending both verify helpers, and reassign main.tscn's theme override to pulse_neocade_theme.tres — completing the FOUND-03 5-direction set and reconnecting the showcase scene's theme reference that Plan 04-01 cleared.**
...
3. **Task 2: Reassign main.tscn theme override to pulse_neocade_theme.tres** — `a39c4aa` (feat)
4. **Task 3: Atomic commit — peer themes + main.tscn reassignment** — `a39c4aa` (feat)
...
- `verify_04_07_task2.ps1` → PASS (`main.tscn` first line is `[gd_scene`; contains the pulse_neocade_theme.tres ext_resource; live `theme = ExtResource(...)` line on the root Control with matching id `1_pulse_theme`; no comment-form placeholder; no reference to the deleted scaffold `neocade_theme.tres`).
...
- [x] `main.tscn` carries the pulse ext_resource + live theme = ExtResource line
