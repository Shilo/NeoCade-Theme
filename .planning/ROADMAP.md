...
**Success Criteria** (what must be TRUE):
  1. **Addon layout exists (flat, single-class, 2026-05-06e):** `addons/neocade_theme/` contains `fonts/`, `icons/`, and `scripts/` subdirectories and only runtime addon resources: **N `.tres` files** (`{name}_neocade_theme.tres`, one per approved direction), `scripts/neocade_theme.gd`, `scripts/neocade_theme_option_button.gd`, `fonts/inter_variable.ttf`, `fonts/inter_ofl.txt`, header FontVariation resources, SVG icons, and Godot import sidecars. Package docs (`README.md`, `docs/usage.md`, `CHANGELOG.md`, `LICENSE.md`) and root `VERSION` stay outside the addon. For the v1 approved set {Pulse, Slate, Bubble, Daybreak, Burst}: **1 theme-engine `.gd` + 1 reusable UI control `.gd` + 5 `.tres`**. **No root `neocade_theme.tres`**. **No `plugin.cfg`**. **No separate `neocade_mobile_theme.tres` file** — mobile is a `@export platform=MOBILE` toggle on the single class.
