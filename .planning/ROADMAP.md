...
**v1 architecture (locked 2026-05-04, simplified 2026-05-06e/f):** `addons/neocade_theme/neocade_theme.gd` declares `@tool class_name NeoCadeTheme extends Theme` as a single concrete, instantiable class. It has 9 `@export` properties total: Core (`base_color`, `accent_color`, `raised`, `platform`) plus Shape (`corner_radius`, `spacing`, `raised_strength`, `focus_thickness`, `outline_width`). Setters trigger `_regenerate_theme()`, which dynamically populates theme entries via `_get_base_color`-style formulas driven by exports instead of `EditorSettings`, with `is_light = base_color.get_luminance() >= 0.5` computed internally. Each direction ships as data only: `[gd_resource type="NeoCadeTheme" format=3]` at `addons/neocade_theme/{name}_neocade_theme.tres`, with its export values and Theme-Editor-authored entry overrides. The addon also ships reusable `addons/neocade_theme/scripts/neocade_theme_option_button.gd` (`@tool class_name NeoCadeThemeOptionButton extends OptionButton`) for editor-authored theme pickers. **No per-direction `.gd` files, no class hierarchy, no subclasses, no `_dev/`, no `themes/` subfolder, no root `neocade_theme.tres`, and no `neocade_mobile_theme.tres`.** `platform=AUTO` auto-detects via `OS.has_feature("mobile")` at runtime. Phase 3.2 validated the underlying dynamic-theme feasibility; the 2026-05-06e/f simplification replaced the earlier superclass/subclass recipe before Phase 4 implementation.
...
**Success Criteria** (what must be TRUE):
  1. **Addon layout exists (flat, single-class, 2026-05-06e):** `addons/neocade_theme/` contains `fonts/`, `icons/`, and `scripts/` subdirectories and only runtime addon resources: `neocade_theme.gd`, its `.uid`, **N `.tres` files** (`{name}_neocade_theme.tres`, one per approved direction), `scripts/neocade_theme_option_button.gd`, `fonts/inter_variable.ttf`, `fonts/inter_ofl.txt`, header FontVariation resources, SVG icons, and Godot import sidecars. Root docs (`README.md`, `ADDON_USAGE.md`, `CHANGELOG.md`, `LICENSE.md`) and root `VERSION` stay outside the addon. For the v1 approved set {Pulse, Slate, Bubble, Daybreak, Burst}: **1 theme-engine `.gd` + 1 reusable UI control `.gd` + 5 `.tres`**. **No root `neocade_theme.tres`**. **No `plugin.cfg`**. **No separate `neocade_mobile_theme.tres` file** — mobile is a `@export platform=MOBILE` toggle on the single class.
