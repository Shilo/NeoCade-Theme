...
- [ ] **Phase 3.3: Theme Direction Research — 5 Flat-MD3 Candidate Directions (INSERTED 2026-05-04 theme-direction insertion)** - **Derives 5 candidate theme directions from scratch** in the flat-MD3 + extruded-flat idiom. Inputs (in priority order): (1) user's new goal — "colorful, flat, modern UI; not outdated 3d/texture/engraving; can be colorful + playful + expressive"; (2) user's hard restrictions — no textures/patterns/embossing/painterly/gradients on chrome; (3) user's exemplar references (hcgamestudios + fajrulaslim itch.io); (4) **user's per-v0-direction reactions as DNA inputs (NOT name carryovers)** — what loved/rejected per Midnight Marquee/Boardwalk Sunset/Cabinet Chrome/Prize Pop Plaza/Orbital Playdeck; (5) Phase 3.1 MD3 + MD3 Expressive findings. Surveys 10-15 commercial flat-MD3 game UI examples for validation. **Produces 5 NEW direction names — MUST NOT be v0 names.** User-approval checkpoint at text-level (names + base/accent colors + personality summaries) before Phase 3.4 mocks them up. Output: `.planning/research/THEME-DIRECTIONS.md` + SOURCES.md Section 14.
...
**Success Criteria** (what must be TRUE):
  ...
  2. **Names MUST be NEW.** Direction names MUST NOT be Midnight Marquee, Boardwalk Sunset, Cabinet Chrome, Prize Pop Plaza, or Orbital Playdeck (those are v0). v0 reactions feed DNA only.
  ...
  4. **Per-v0-direction reaction → DNA mapping table.** Explicit table showing what user loved (extracted as DNA) vs rejected (dropped) per v0 direction. Each new direction's rationale references which DNA elements informed it.
  ...
  7. **User-approval checkpoint.** After the 5 directions are documented, agent presents them via AskUserQuestion. User approves all 5 / revises specific directions (max 2 revision rounds) / rejects all 5 (escalation discussion). Approval is text-level only — no mockups exist yet at this phase.
  8. **SOURCES.md Section 14 added** with adopt/reject/open synthesis.
  9. **No mockup commits, no `.tres` commits.** Phase 3.3 is research-only.
**Plans**: TBD
**UI hint**: no
