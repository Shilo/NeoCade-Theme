...
- [x] **Phase 9: Showcase + Token Gallery + Theme Picker** - editor-authored `res://showcase/showcase.tscn` with 9 sections + reusable `NeoCadeThemeOptionButton` dropdown that discovers themes from `addons/neocade_theme/`, sorts themes alphabetically, appends optional `None`, and emits `theme_selected(theme, index)` after applying a selection; raised/platform remain resource exports, not showcase runtime controls (completed 2026-05-07; corrected 2026-05-08)
...
**Success Criteria** (what must be TRUE):
  1. **Addon layout exists (flat, single-class, 2026-05-06e; editor save support added 2026-05-08):** `addons/neocade_theme/` contains `fonts/`, `icons/`, and `scripts/` subdirectories and addon resources: **N `.tres` files** (`{name}_neocade_theme.tres`, one per approved direction), `scripts/neocade_theme.gd`, `scripts/neocade_theme_option_button.gd`, focused editor-save support (`plugin.cfg`, `scripts/neocade_theme_plugin.gd`, `scripts/neocade_theme_resource_format_saver.gd`), `fonts/inter_variable.ttf`, `fonts/inter_ofl.txt`, header FontVariation resources, SVG icons, and Godot import sidecars. Package docs (`README.md`, `docs/usage.md`, `CHANGELOG.md`, `LICENSE.md`) and root `VERSION` stay outside the addon. For the v1 approved set {Pulse, Slate, Bubble, Daybreak, Burst}: **No root `neocade_theme.tres`**. **No separate `neocade_mobile_theme.tres` file** — mobile is a `@export platform=MOBILE` toggle on the single class.
...
**Goal**: Build `res://showcase/showcase.tscn` — an editor-authored showcase scene that visually proves every Godot Control is themed in v1, doubles as the QA forcing function, and includes a dropdown so consumers can compare which approved direction `.tres` is active plus NeoCade vs a null target theme. Raised/platform variations remain `NeoCadeTheme` resource exports previewed through the inspector or consumer code, not showcase runtime controls.
**Depends on**: Phase 8
**Requirements**: SHOW-01, SHOW-02, SHOW-03, SHOW-04, SHOW-05, SHOW-06, SHOW-07, SHOW-08
**Success Criteria** (what must be TRUE):
  1. **Showcase scene exists and is project main scene:** `res://showcase/showcase.tscn` is set as the project's main scene; uses NeoCade theme — specifically, preloads the **recommended starter direction's `.tres`** (e.g., `res://addons/neocade_theme/slate_neocade_theme.tres` if Slate is the recommended starter picked at the Phase 3.4 gate) as project theme (or per-scene `theme` override if leak avoidance preferred — decision documented); opens to a fullscreen Control root with all 9 sections visible/scrollable. Per flat-layout 2026-05-06d, no root `neocade_theme.tres` exists.
  ...
  3. **Theme picker is dropdown-only; showcase behavior script is minimal:** The scene provides a Theme picker that cycles through the 5 approved direction `.tres` files (`pulse_neocade_theme.tres`, `slate_neocade_theme.tres`, `bubble_neocade_theme.tres`, `daybreak_neocade_theme.tres`, `burst_neocade_theme.tres`) + `None` (`null` target theme). The rest of the showcase UI is serialized scene nodes, not constructed by script; `showcase/showcase.gd` only handles behavior glue such as the scoreboard Window close/reopen path. Per flat-layout 2026-05-06d, no root `neocade_theme.tres` is included in the theme picker.
...
**Success Criteria** (what must be TRUE for the autonomous Phase 10 closeout):
  ...
  7. **Theme inspector workaround documented (QA-06):** `docs/usage.md` notes the active issue #115500 — do NOT edit theme resources through a Control inspector context menu. Safe authoring paths are the dedicated Theme editor, the 9 exported `NeoCadeTheme` properties on direction resources, and formula edits in `addons/neocade_theme/scripts/neocade_theme.gd`.
**Plans**: 1 plan
...
**Reference workflow:** [Shilo/PentaTile release.yml](https://github.com/Shilo/PentaTile/blob/main/.github/workflows/release.yml) — adapt the same 11-step pattern. Notable adaptations: version source is root `VERSION` (single-line file) instead of addon `plugin.cfg`; add a Godot Web export step that produces a separate web-build zip uploaded as a release asset.
**Success Criteria** (what must be TRUE):
