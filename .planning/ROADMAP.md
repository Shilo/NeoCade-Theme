...
- [ ] **Phase 3.1: Source-Dive — Material Design 3, MD3 Expressive, and Flat-3D Game UI Research (INSERTED 2026-05-04)** - Exhaustive research on Material Design 3 (current spec: color/type/shape/motion/components/dynamic-color/surface-tinting), MD3 Expressive (newer evolution), and the "Flat-3D Game UI" / "Extruded Flat UI" pattern as exemplified by the user's itch.io references. Produces `.planning/research/MD3-RESEARCH.md` + `.planning/research/FLAT-3D-UI-RESEARCH.md`. No mockups, no `.tres` work; pure research.
- [ ] **Phase 3.2: Visual Direction Mockup + Approval Gate (Flat / Extruded-Flat) (INSERTED 2026-05-04)** - Replaces redirected Phase 3 with new constraints: flat MD3 visual identity, optional extruded-flat 3D raised variation, NO textures/patterns/embossing/leather backgrounds. Produces 5 new themes (informed by Phase 3.1 research + user feedback on the original 5); each finalized theme has 4 `.tres` variations (flat × raised × desktop × mobile). Architecture supports undefined number of themes; v1 ships N approved themes × 4 = 4N `.tres` files. Hard gate before Phase 4.
- [ ] **Phase 4: Foundation — Tokens, Fonts, Icons, Scaffold + `@tool` Generator (UPDATED for variation matrix)** - TokenSet single-source-of-truth, bundled fonts, bespoke icons, generator producing **N themes × 4 variations = 4N `.tres` files** from a single TokenSet matrix (per-theme palette + per-variation overrides). Replaces the original "two `.tres` files" scope.
...
### Phase 3.2: Visual Direction Mockup + Approval Gate (Flat / Extruded-Flat) (INSERTED 2026-05-04)
**Goal**: Replace the redirected Phase 3 visual direction with **flat MD3 / MD3 Expressive** design language, optional **extruded-flat raised** variation per the Flat-3D Game UI pattern, and approval-gate 5 themes × 4 variations (flat × raised × desktop × mobile) before any `.tres` styling work begins. Hard gate. **Architecture supports undefined number of themes** — v1 ships N approved themes × 4 variations.
**Depends on**: Phase 3.1 (research output is the design language input)
**Requirements**: DESIGN-NEW-01..06 (5 themes × 4 variations), TOKEN-NEW-01..10 (token spec updated for theme + variation matrix), DESIGN-NEW-MOBILE-01..03 (mobile variant spec for tap targets / type scale / spacing), TOKEN-MATRIX-01 (variation matrix architecture)
**Success Criteria** (what must be TRUE):
  1. **5 named themes** produced as flat MD3 mockups (no textures, no patterns, no embossing, no painterly backgrounds, no gradients on chrome). Theme names may overlap with the historical Phase 3 direction names (Midnight Marquee, Cabinet Chrome, Prize Pop Plaza, Orbital Playdeck) where the user's feedback indicated the colors/personality were liked, but each is **fully redesigned through the flat-MD3 filter**. Boardwalk Sunset is dropped per user rejection. The 5th theme is research-derived.
  2. **Each theme has 4 variation mockups:** `flat-desktop`, `flat-mobile`, `raised-desktop` (extruded-flat), `raised-mobile` (extruded-flat). All four variations share the theme's palette/typography/shape; the raised variation adds offset darker duplicate-shape depth (per FLAT-3D-UI-RESEARCH.md), the mobile variation increases tap-target sizes per CROSS-PLATFORM.md mobile spec.
  3. **Step 1 — Concept-flat boards:** 5 lighter-weight concept boards (flat MD3 only, no raised) shown to user for direction selection. User picks 1-3 finalists. May reference historical Phase 3 concept images for color/personality DNA but must demonstrably re-render in flat MD3.
  4. **Step 2 — Finalist full-fidelity mockups:** Selected finalists get full-fidelity HTML mockups for all 4 variations (flat × raised × desktop × mobile), every Control × every state combination (including state combinations Pitfall 1.1 surfaced: `pressed_focus`, `checked_focus`, `hover_pressed`).
  5. **Step 3 — Approval & DESIGN_TOKENS.md:** User approves N final themes (N ≥ 1; user's call). `DESIGN_TOKENS.md` finalized: token-matrix architecture documenting the per-theme color/type/shape blocks + per-variation override blocks (flat: `shadow_size = -1`, raised: `shadow_color`/`shadow_offset` per Flat-3D-UI pattern; desktop sizes vs mobile +50% spacing / 48px tap targets).
  ...
  7. **Hard blocker enforcement:** No `.tres` styling commits exist on the branch when Phase 3.2 closes; Phase 4 cannot start until Step 3 user approval is logged in writing.
  8. **Historical preservation:** All Phase 3 (v0) artifacts remain in `.planning/mockups/concepts/` + `.planning/mockups/03-direction-boards.*`. Phase 3.2 outputs go to `.planning/mockups/3.2/` (or similar) so the two iterations don't conflict.
**Plans**: TBD
**UI hint**: yes

### Phase 4: Foundation — Tokens, Fonts, Icons, Scaffold + `@tool` Generator (UPDATED for variation matrix)
**Goal**: Build the structural foundation — addon directory layout, bundled OFL fonts, bespoke SVG icons, and the `@tool` generator that produces **N themes × 4 variations = 4N `.tres` files** (where each theme has `flat-desktop`, `flat-mobile`, `raised-desktop`, `raised-mobile` variations per the Flat-3D Game UI pattern from Phase 3.1 / 3.2) from a single TokenSet matrix — so every subsequent phase populates ONE source of truth and drift across themes / variations / platforms is structurally impossible. **Architecture supports undefined number of themes**; v1 ships the N themes user approved at the Phase 3.2 gate. Each theme contributes a per-theme block to TokenSet (palette, optional shape/personality overrides); each variation is a per-variation override block (flat: no shadow, `shadow_size = -1`; raised: `shadow_color` + `shadow_offset` per Flat-3D-UI pattern; mobile: +50% spacing on space.4+, 48px tap-target floor, body 16px vs desktop 14px).
**Depends on**: Phase 3.2 (mockup approval is a hard prerequisite — Phase 3.2 replaces the redirected Phase 3 gate)
