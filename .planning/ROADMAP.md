...
- [x] **Phase 9: Showcase + Token Gallery + Theme Picker** - editor-authored `res://showcase/showcase.tscn` with 9 sections + reusable `NeoCadeThemeOptionButton` dropdown that discovers themes from `addons/neocade_theme/`, keeps optional `None` first, sorts themes alphabetically, preserves selection by resource path, and emits `theme_selected(index, theme_path, theme)` after applying a selection; raised/platform remain resource exports, not showcase runtime controls (completed 2026-05-07; corrected 2026-05-08)
