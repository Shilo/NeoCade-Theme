...
NeoCade is a feature-complete coverage project, not a velocity project. The journey is: dissect prior-art (`godot-minimal-theme` `.tres`, LDtk source) → research **flat MD3 / MD3 Expressive / Flat-3D Game UI** visual language (Phase 3.1) **in parallel** with **Godot dynamic theme architecture** (Phase 3.2 — feasibility validation of `NeoCadeTheme` superclass + per-theme subclass pattern) → produce **flat-MD3 visual direction with dynamic-theme demonstration** through user-approved mockups (Phase 3.3 hard gate; replaces redirected Phase 3) → implement the **`NeoCadeTheme` superclass** (validated by Phase 3.2 spike) + per-theme subclasses + fonts + icons (Phase 4) → author every Godot 4.6 user-facing Control class across all states (Phases 5-7) → fill mobile + raised token deltas and audit tap targets (Phase 8) → showcase scene with theme toggle (Phase 9) → exhaustive QA + cross-platform export validation across all 6 Godot targets (Phase 10) → GitHub Actions release pipeline (Phase 11). 14 phases (was 11; Phase 3 redirected; Phase 3.1 + 3.2 + 3.3 inserted 2026-05-04). **No Asset Library submission in v1.** Mockup approval is a hard blocker between Phase 3.3 and Phase 4. Mobile + raised + dynamic-theme architecture are all v1 must-haves. All 6 Godot export targets are v1 must-haves.
...
- [ ] **Phase 3.3: Visual Direction Mockup + Approval Gate (Flat / Extruded-Flat) (INSERTED 2026-05-04; was Phase 3.2 before architecture revision)** - Replaces redirected Phase 3 with new constraints: flat MD3 visual identity, optional extruded-flat raised variation, NO textures/patterns/embossing. **Mockups demonstrate the dynamic `NeoCadeTheme` superclass** — each finalist shown in 4 export-state configurations (flat × raised × desktop × mobile) side-by-side, proving one subclass `.tres` produces all 4 dynamically. Produces 5 new themes (informed by Phase 3.1 visual research + Phase 3.2 architecture findings + user feedback on the original 5). v1 ships N approved theme subclasses × 1 `.tres` each (consumer toggles exports for variations). Hard gate before Phase 4.
- [ ] **Phase 4: Foundation — `NeoCadeTheme` Superclass + Per-Theme Subclasses + Fonts + Icons (UPDATED for dynamic architecture)** - Implements the `NeoCadeTheme` superclass (validated by Phase 3.2 spike) at `addons/neocade_theme/_dev/neocade_theme.gd` with `@tool` + `@export` props (`base_color`, `accent_color`, `raised`, `platform`). Per-theme subclasses (`PrizePopPlazaNeoCadeTheme`, etc.) extend the superclass with personality (corner radii, outline widths, color tint formulas). v1 ships N approved theme subclass `.tres` files at `addons/neocade_theme/themes/`. Bundles Inter Variable Roman fonts + bespoke SVG icons. Replaces the original "two static `.tres` files generated from TokenSet" scope.
...
### Phase 3.3: Visual Direction Mockup + Approval Gate (Flat / Extruded-Flat) (INSERTED 2026-05-04; was Phase 3.2 before architecture revision)
**Goal**: Replace the redirected Phase 3 visual direction with **flat MD3 / MD3 Expressive** design language, optional **extruded-flat raised** variation per the Flat-3D Game UI pattern, and approval-gate **5 themes demonstrated through the dynamic `NeoCadeTheme` superclass** — each finalist shown in 4 export-state configurations (flat × raised × desktop × mobile) side-by-side, proving one subclass produces all 4 dynamically. Hard gate. **Architecture supports undefined number of themes** — v1 ships N approved theme subclasses × 1 `.tres` each (consumer toggles exports for variations).
**Depends on**: Phase 3.1 (visual design language) + Phase 3.2 (architecture feasibility — must validate before mockups assume dynamic regeneration works)
...
**Success Criteria** (what must be TRUE):
  1. **5 named themes** produced as flat MD3 mockups (no textures, no patterns, no embossing, no painterly backgrounds, no gradients on chrome). Theme subclass names use Pascal case ending in `NeoCadeTheme` (e.g., `PrizePopPlazaNeoCadeTheme`); files use snake case (`prize_pop_plaza_neocade_theme.tres`). Theme names may overlap with the historical Phase 3 direction names (Midnight Marquee, Cabinet Chrome, Prize Pop Plaza, Orbital Playdeck) where the user's feedback indicated the colors/personality were liked, but each is **fully redesigned through the flat-MD3 filter**. Boardwalk Sunset is dropped per user rejection. The 5th theme is research-derived from Phase 3.1.
  2. **4-grid mockup format per finalist:** Each finalist mockup demonstrates the SAME subclass in 4 export-state configurations side-by-side: top-left `raised=false, platform=DESKTOP`, top-right `raised=false, platform=MOBILE`, bottom-left `raised=true, platform=DESKTOP`, bottom-right `raised=true, platform=MOBILE`. Plus 1 row showing `base_color`/`accent_color` override examples. **All four cells share one subclass** (proving the dynamic system).
  3. **Step 1 — Concept-flat boards:** 5 lighter-weight concept boards (flat MD3 only, no raised yet) shown to user for direction selection. User picks 1-3 finalists.
  4. **Step 2 — Finalist full-fidelity mockups:** Selected finalists get full-fidelity HTML mockups in the 4-grid format, every Control × every state combination (including Pitfall 1.1's combinations: `pressed_focus`, `checked_focus`, `hover_pressed`).
  5. **Step 3 — Approval & DESIGN_TOKENS.md:** User approves N final themes (N ≥ 1). DESIGN_TOKENS.md finalized as **subclass-recipe-oriented**, NOT TokenSet-block-oriented (per architecture revision). Documents per theme: subclass class_name, default `base_color`, default `accent_color`, personality overrides (corner radii, outline widths, surface-ramp formula `(brightness_offset, saturation_multiplier)` parameters per stop, accent tint formula parameters). Plus shared base contract (Inter type scale, M3 state-layer model, focus-ring strategy, anti-texture rules) and variation override blocks (flat / raised / desktop / mobile / AUTO detection).
  6. **Anti-cyberpunk + anti-texture filter pass:** Each finalist is audited against the redirect's hard rules (no textures, no patterns, no embossing, no leather/wood/painterly, no gradients on chrome) before approval. Audit output committed.
  7. **Hard blocker enforcement:** No `.tres` styling commits under `addons/neocade_theme/` exist on the branch when Phase 3.3 closes; Phase 4 cannot start until Step 3 user approval is logged in writing.
  8. **Historical preservation:** All Phase 3 (v0) artifacts remain in `.planning/mockups/concepts/` + `.planning/mockups/03-direction-boards.*`. Phase 3.3 outputs go to `.planning/mockups/3.3/` so the two iterations don't conflict.
**Plans**: TBD
**UI hint**: yes
...
**Depends on**: Phase 3.3 (mockup approval is a hard prerequisite) + Phase 3.2 (architecture recipe + spike feasibility validation must have passed)
