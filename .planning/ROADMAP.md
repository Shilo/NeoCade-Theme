...
**v1 architecture (updated 2026-05-13):** `addons/neocade_theme/scripts/neocade_theme.gd` declares `@tool class_name NeoCadeTheme extends Theme` as a single concrete, instantiable class. It has 11 `@export` properties total: top-level exports (`style`, `raised`, `platform`), the Style Overrides group (`source_color`, `corner_radius`, `spacing`, `raised_strength`, `focus_thickness`, `outline_width`), and the Advanced exports (`use_runtime_popup_selection_icons`, `texture_cache`). Setters trigger `_regenerate_theme()`, which dynamically populates theme entries from per-style source-color role palettes. `is_light` is computed from the generated surface role. The addon ships one canonical `addons/neocade_theme/neocade_theme.tres` resource; `NeoCadeTheme.Style` selects Bubble, Burst, Daybreak, Pulse, Slate, or Custom. The addon also ships reusable `addons/neocade_theme/scripts/neocade_theme_option_button.gd` (`@tool class_name NeoCadeThemeOptionButton extends OptionButton`) for editor-authored style pickers; it appends optional `None` after alphabetized styles, resets the selected style preset source/shape values, preserves independent `raised` / `platform` / Advanced toggles, and emits `theme_selected(theme, index)` after applying a selection. **No per-direction `.gd` files, no per-style `.tres` files, no class hierarchy, no subclasses, no `_dev/`, no `themes/` subfolder, and no `neocade_mobile_theme.tres`.** `platform=AUTO` auto-detects via `OS.has_feature("mobile")` at runtime. The old public `base_color` / `accent_color` API was intentionally removed without backwards compatibility.
...
1. **C4 — HSV value-darken depth formula.** Replace `_raised_depth_color`
   with `Color.from_hsv(h, s, v * (1.0 - strength))` so hue/saturation
   are preserved and colored buttons keep a same-family hard underside.
   Follow-up tuning allows built-in styles to set hidden
   `shape.raised_depth_darken` and `shape.raised_depth_scale` values:
   Bubble keeps a chunkier but softer candy-game underside, Burst becomes
   deliberately bolder, and Slate stays restrained. No public exports
   added. ~30 min.
...
### Phase 4: Foundation — Single `NeoCadeTheme` Class + Canonical Style Resource + Fonts + Icons (UPDATED 2026-05-13)
**Goal**: Build the structural foundation — addon directory layout, bundled OFL font, bespoke SVG icons, and the **single concrete `NeoCadeTheme` class** that dynamically regenerates theme entries from the finalized exported properties. v1 ships one canonical `neocade_theme.tres` resource with built-in Bubble, Burst, Daybreak, Pulse, and Slate styles. At runtime, consumers load `neocade_theme.tres`, optionally toggle `style` / `raised` / `platform` / `source_color`, and the class regenerates all theme entries to match. `platform=AUTO` auto-detects via `OS.has_feature("mobile")`.
...
**Success Criteria** (what must be TRUE):
  ...
  4. **`NeoCadeTheme` class works (single concrete class, 11-property `@export` set, 2026-05-13):** `addons/neocade_theme/scripts/neocade_theme.gd` declares `@tool class_name NeoCadeTheme extends Theme` with **11 `@export` properties total**. **Top level (3):** `style`, `raised`, `platform`. **Style Overrides group (6):** `source_color`, `corner_radius`, `spacing`, `raised_strength`, `focus_thickness`, `outline_width`. **Advanced group (2):** `use_runtime_popup_selection_icons`, `texture_cache`. `style` is the explicit direction selector and `Style.CUSTOM` is manual/custom mode. Setters regenerate theme entries via `_regenerate_theme()`. The old `base_color` / `accent_color` API is removed without backwards compatibility.
...
- [x] 12-02-wave-1-c4-hsv-depth-PLAN.md — Wave 1: rewrite _raised_depth_color body with the D-12.02 HSV value-darken formula; follow-up tuning adds hidden per-style depth scale/darken values while preserving the public signature/export contract.

**Wave 2** *(blocked on Wave 1 completion)*
- [x] 12-03-wave-2-c2prime-accent-rebinds-PLAN.md — Wave 2: rebind 6 BINDING_TABLE rows (TabBar/TabContainer tab_selected, ItemList/Tree selected/selected_focus) to role_primary accent stripes — the headline fix (mid-phase fallback boundary per D-12.20).

**Wave 3** *(blocked on Wave 2 completion)*
- [x] 12-04-wave-3-c6-per-direction-PLAN.md — Wave 3: per-direction signatures (Slate hairlines, Bubble ≥26 radius floor, Daybreak flat outline, Burst oversized primaries) + Pulse showcase Kicker + SC#4 greyscale thumbnail user attestation.
...
**Wave 3** *(blocked on Wave 2 completion)*
- [x] 13-04-PLAN.md — Wave 3: add 10th showcase section + README Role Variations (opt-in) docs + run full Phase 13 Nyquist suite for closeout evidence. (completed 2026-05-11)
