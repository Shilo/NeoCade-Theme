...
**v1 architecture (locked 2026-05-04, simplified 2026-05-06e/f):** `addons/neocade_theme/scripts/neocade_theme.gd` declares `@tool class_name NeoCadeTheme extends Theme` as a single concrete, instantiable class. It has 9 `@export` properties total: Core (`base_color`, `accent_color`, `raised`, `platform`) plus Shape (`corner_radius`, `spacing`, `raised_strength`, `focus_thickness`, `outline_width`). Setters trigger `_regenerate_theme()`, which dynamically populates theme entries via `_get_base_color`-style formulas driven by exports instead of `EditorSettings`, with `is_light = base_color.get_luminance() >= 0.5` computed internally. Each direction ships as data only: `[gd_resource type="NeoCadeTheme" format=3]` at `addons/neocade_theme/{name}_neocade_theme.tres`, with its export values and Theme-Editor-authored entry overrides. The addon also ships reusable `addons/neocade_theme/scripts/neocade_theme_option_button.gd` (`@tool class_name NeoCadeThemeOptionButton extends OptionButton`) for editor-authored theme pickers. **No per-direction `.gd` files, no class hierarchy, no subclasses, no `_dev/`, no `themes/` subfolder, no root `neocade_theme.tres`, and no `neocade_mobile_theme.tres`.** `platform=AUTO` auto-detects via `OS.has_feature("mobile")` at runtime. Phase 3.2 validated the underlying dynamic-theme feasibility; the 2026-05-06e/f simplification replaced the earlier superclass/subclass recipe before Phase 4 implementation.
...
- [x] **Phase 4: Foundation — Single `NeoCadeTheme` Class + Data-Only Direction `.tres` Files + Fonts + Icons (UPDATED 2026-05-06f)** - Implements `addons/neocade_theme/scripts/neocade_theme.gd` as the single concrete `@tool class_name NeoCadeTheme extends Theme` with the finalized 9-property export set. Ships 5 data-only direction resources at the addon root (`pulse_neocade_theme.tres`, `slate_neocade_theme.tres`, `bubble_neocade_theme.tres`, `daybreak_neocade_theme.tres`, `burst_neocade_theme.tres`) plus Inter Variable Roman and bespoke SVG icons. No subclasses, no per-direction `.gd`, no `_dev/`, no `themes/`, no root `neocade_theme.tres`, no `neocade_mobile_theme.tres`.
...
- [x] **Phase 9: Showcase + Token Gallery + Theme Picker** - editor-authored `res://main.tscn` with 9 sections + reusable `NeoCadeThemeOptionButton` dropdown that discovers themes from `addons/neocade_theme/`, keeps optional `Default` first, then sorts themes alphabetically; raised/platform remain resource exports, not showcase runtime controls (completed 2026-05-07; corrected 2026-05-08)
...
**Success Criteria** (what must be TRUE):
  1. **Addon layout exists (flat, single-class, 2026-05-06e):** `addons/neocade_theme/` contains `fonts/`, `icons/`, and `scripts/` subdirectories and only runtime addon resources: **N `.tres` files** (`{name}_neocade_theme.tres`, one per approved direction), `scripts/neocade_theme.gd`, `scripts/neocade_theme_option_button.gd`, `fonts/inter_variable.ttf`, `fonts/inter_ofl.txt`, header FontVariation resources, SVG icons, and Godot import sidecars. Root docs (`README.md`, `ADDON_USAGE.md`, `CHANGELOG.md`, `LICENSE.md`) and root `VERSION` stay outside the addon. For the v1 approved set {Pulse, Slate, Bubble, Daybreak, Burst}: **1 theme-engine `.gd` + 1 reusable UI control `.gd` + 5 `.tres`**. **No root `neocade_theme.tres`**. **No `plugin.cfg`**. **No separate `neocade_mobile_theme.tres` file** — mobile is a `@export platform=MOBILE` toggle on the single class.
...
- Preserve the dynamic architecture: one concrete `addons/neocade_theme/scripts/neocade_theme.gd`, five data-only direction `.tres` resources, and the locked 9-export surface.
...
**Goal**: Build `res://main.tscn` — an editor-authored showcase scene that visually proves every Godot Control is themed in v1, doubles as the QA forcing function, and includes a dropdown so consumers can compare which approved direction `.tres` is active plus NeoCade vs Godot default. Raised/platform variations remain `NeoCadeTheme` resource exports previewed through the inspector or consumer code, not showcase runtime controls.
**Depends on**: Phase 8
...
**Success Criteria** (what must be TRUE):
  ...
  3. **Theme picker is dropdown-only:** The scene provides a Theme picker that cycles through the 5 approved direction `.tres` files (`pulse_neocade_theme.tres`, `slate_neocade_theme.tres`, `bubble_neocade_theme.tres`, `daybreak_neocade_theme.tres`, `burst_neocade_theme.tres`) + Godot default. The rest of the showcase UI is serialized scene nodes, not constructed by script. Per flat-layout 2026-05-06d, no root `neocade_theme.tres` is included in the theme picker.
...
Plans:
- [x] `09-01-PLAN.md` — Implement the showcase scene with nine editor-authored sections, dropdown-only direction/default theme picker, token gallery, and coverage strip.
**UI hint**: yes
...
**Success Criteria** (what must be TRUE for the autonomous Phase 10 closeout):
  ...
  7. **Theme inspector workaround documented (QA-06):** CONTRIBUTING.md notes the active issue #115500 — do NOT edit theme resources through a Control inspector context menu. Safe authoring paths are the dedicated Theme editor, the 9 exported `NeoCadeTheme` properties on direction resources, and formula edits in `addons/neocade_theme/scripts/neocade_theme.gd`.
**Plans**: 1 plan
...
**Success Criteria** (what must be TRUE):
  ...
  5. **Addon zip via `git archive`:** workflow runs `git archive --format=zip --prefix="neocade_theme-v<VERSION>/" -o "neocade_theme-v<VERSION>.zip" "v<VERSION>" -- addons/neocade_theme/ README.md ADDON_USAGE.md CHANGELOG.md LICENSE.md VERSION` — only tracked files at the tagged commit, with runtime addon files under `addons/neocade_theme/` and docs/licenses at the zip root. (Pitfall #11 from PentaTile: this excludes `.godot/`, build artifacts, untracked.)
