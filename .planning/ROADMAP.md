...
- [ ] **Phase 11: Distribution — Asset Library Submission** - Asset Library policy re-verification, README, OFL.txt attribution, fresh-install dry-run, v1.0.0 ship
...
**Success Criteria** (what must be TRUE):
  ...
  4. **Step 2 — Typography approval:** 2 typography mockups (Variant A: Inter-only no Outfit; Variant B: Inter body + Outfit headings — recommended per Conflict 1 revision) saved alongside the chosen palette; mockups show the full M3 type-scale spine (display/headline/title/body/label/code) with the chosen palette applied; user selects ONE at Step 2 approval.
...
### Phase 11: Distribution — Asset Library Submission
**Goal**: Ship v1.0.0 — package the addon for Godot Asset Library submission, re-verify current Asset Library policy at submission time (do not rely on stale training-data docs), and produce all consumer-facing documentation.
**Depends on**: Phase 10
**Requirements**: RES-05, DIST-01, DIST-02, DIST-03, DIST-04, DIST-05, DOCS-04
**Success Criteria** (what must be TRUE):
  1. **Asset Library current policy re-verified (RES-05, DIST-05):** Context7 MCP queried at submission time for current Godot Asset Library submission requirements; deltas vs initial-pass docs noted; no submission proceeds on stale information.
  2. **Asset Library submission package complete (DIST-01):** Square 128×128 PNG icon produced and hosted at a URL Asset Library can reach; README documents both install paths (project theme + per-scene theme + optional editor theme) with the editor-leak caveat per PITFALLS 2.2 + EDITOR-COVERAGE.md; "not an editor plugin" note prominent; license attributions surfaced.
  3. **License + changelog files in place (DIST-02, DIST-03):** Combined `OFL.txt` covers all 4 bundled fonts (Inter + Outfit + Noto Sans + JetBrains Mono) with each font's Reserved Font Name notice block + copyright lines; `LICENSE.md` for theme code (MIT or CC-BY recommended); `CHANGELOG.md` initial entry for v1.0.0 documents every shipped feature + every documented limitation (Inter Italic deferred to v1.x, CJK not bundled, real-device mobile QA status per UD-5, screen-reader QA status per UD-6).
  4. **README is comprehensive (DOCS-04, DIST-04):** Project description, install paths, usage examples, cross-platform support summary, mobile variant usage, accessibility notes, CJK override pattern (UD-2), editor-coverage map link (EDITOR-COVERAGE.md), license, attributions, link to GitHub repo.
  5. **Submission-ready confirmation:** Fresh-install dry-run from Phase 10 still passes against the packaged addon; Asset Library submission is one click away; v1.0.0 git tag created.
**Plans**: TBD
...
|-------|----------------|--------|-----------|
| 1. Source-Dive: godot-minimal-theme | 0/TBD | Not started | - |
...
- UD-4 (Inter Italic v1 vs v1.x) — RESOLVED in Conflict 1 revision (Outfit ships v1, Inter Italic deferred); user can override at Phase 3 typography mockup gate
