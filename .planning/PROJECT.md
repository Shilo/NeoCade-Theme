...
NeoCade is a Godot 4.6 native UI Theme system, distributed as a drop-in addon, that styles every built-in Control with a **flat Material Design 3 / MD3 Expressive aesthetic** — modern, colorful, expressive, accessibility-first, with optional "extruded flat 3D" raised variation per the Flat-3D Game UI pattern. It works universally across the Godot Editor and game runtime, and is designed to scale from desktop to mobile. **Architecture (updated 2026-05-13):** `@tool class_name NeoCadeTheme extends Theme` — single concrete class (NOT abstract; users can instantiate to make their own custom themes). Lives at `addons/neocade_theme/scripts/neocade_theme.gd`. The canonical resource lives at `addons/neocade_theme/neocade_theme.tres`. The class has 11 `@export` properties total: **Top level (3)** — `style: {BUBBLE, BURST, DAYBREAK, PULSE, SLATE, CUSTOM}`, `raised: bool`, `platform: {DESKTOP, MOBILE, AUTO}`. **Style Overrides group (6, collapsible @export_group("Style Overrides"))** — `source_color: Color`, `corner_radius: int`, `spacing: int`, `raised_strength: int`, `focus_thickness: int`, `outline_width: int`. **Advanced group (2)** — `use_runtime_popup_selection_icons: bool`, which allows consumers to disable the tiny runtime-generated PopupMenu check/radio icons if they prefer static SVG fallback over perfect menu-state color consistency, and `texture_cache: bool`, which keeps loaded/generated textures across regenerations for faster live tweaking when enabled. `style` is the explicit direction selector; built-in styles apply the approved direction values and hidden direction personality, while `source_color` is the single public color input each style maps into its own surface/action/menu/input/selection/range/toggle/positive/danger role palette. `Style.CUSTOM` is the manual/custom mode. Setters on exported properties trigger `_regenerate_theme()` which dynamically populates derived theme entry color/state values from the source-color role-palette formulas. **Dark/light handling is luminance-derived** (NOT a separate toggle): `var is_light: bool = surface_fill.get_luminance() >= 0.5` is computed from the generated surface role in `_regenerate_theme()`. v1 remains dark-first, with Bubble intentionally using dark shell plus light islands. **No production subclasses. No per-direction `.gd` files. No per-style `.tres` files.** Reusable scripts live under `addons/neocade_theme/scripts/`; `fonts/` and `icons/` remain asset subfolders. Pulse is the recommended starter style and showcase default, but it has no architectural privilege. Consumer toggles exports for flat/raised + desktop/mobile/AUTO variations. `platform=AUTO` auto-detects via `OS.has_feature("mobile")` at runtime; `DESKTOP` and `MOBILE` are forced sizes. Future variants plug in as new enum styles or consumer-authored `Style.CUSTOM` resources. The old `base_color` / `accent_color` public API was intentionally removed without backwards compatibility.
...
A drop-in Godot 4.6 **flat MD3 / MD3 Expressive Theme system** that styles **every** built-in Control to a Godot Minimal Theme bar of feature-completeness, with a colorful, expressive, professional, accessible, modern visual identity — universal across editor and runtime — installable as a single addon. **v1 ships one canonical `.tres` theme resource** at `res://addons/neocade_theme/neocade_theme.tres`, of type `NeoCadeTheme`, with built-in styles for Bubble, Burst, Daybreak, Pulse, and Slate. Consumers toggle `style` / `source_color` / `raised` / `platform` exports at use-time; the single `NeoCadeTheme` class regenerates all theme entries to match. Drift is structurally constrained because all variations come from one class and the same 11-property export contract. Architecture supports future styles and consumer-authored custom resources.

If everything else fails, this single deliverable must work: a polished, feature-complete `res://addons/neocade_theme/neocade_theme.tres` of type `NeoCadeTheme` that "just works" when applied to any Godot Control tree, with consumer-tunable `style` / `source_color` / `raised` / `platform` exports.
...
- **Type variations**: the live `TYPE_VARIATIONS` registry contains 66 entries as of 2026-05-13: core runtime variations, 13 opt-in role/action/surface-text variations, and editor/integration variations. `PrimaryButton` is the single affirmative/high-emphasis Button variation; `DangerButton` is the single destructive Button variation.
- **Showcase scene + theme picker**: `showcase/showcase.tscn` is an editor-authored Control tree for the 10-section showcase, Godot-null-theme comparison, direction picker, raised toggle, platform selector, and Role Variations tab. The reusable `class_name NeoCadeThemeOptionButton` at `addons/neocade_theme/scripts/neocade_theme_option_button.gd` lists built-in `NeoCadeTheme.Style` values alphabetically from the canonical resource, appends optional `None`, and applies selection to an exported target or scene root. `None` applies `null` to the target theme via the `allow_no_theme` / "Allow No Theme" export. The picker emits `theme_selected(theme, index)` after applying a theme; `None` emits `null`. `showcase/showcase.gd` is limited to showcase behavior glue such as scoreboard Window open/close handling and the runtime raised/platform controls. The showcase UI itself must not be created by a runtime builder script.
- **Global usage helper**: `addons/neocade_theme/scripts/neocade_theme_autoload.gd` is bundled as an optional drop-in autoload. It calls `NeoCadeTheme.apply_globally()` and merges the canonical theme into `ThemeDB.default_theme`, avoiding the Godot `[gui] theme/custom` debugger-spam issue documented in README/usage docs.
...
- [x] **Phase 12 — Signature Visual Moves: Defaults**: C4 HSV raised-depth tuning, C2' accent expansion, and C6 per-style signature moves are implemented. The former 12-export contract was superseded by Phase 14's source-color rework.
- [x] **Phase 14 — Source Color Role Palette Rework**: `base_color` / `accent_color` were removed; `source_color` now drives per-style role palettes and contrast-safe `on_*` foregrounds without backwards compatibility.
- [x] **Phase 13 — Role Variations: Opt-Ins**: 4 role Label variations + 5 role Panel variations + the 10th Role Variations showcase section are implemented. Role variations activate only when consumers set `theme_type_variation`.
...
> Note on mobile variant: previously listed as Out of Scope; **moved into v1 scope on 2026-05-04** and implemented as `platform=MOBILE` / `platform=AUTO` on `NeoCadeTheme`. See `.planning/MOBILE-DESIGN-SPEC.md` and `.planning/research/CROSS-PLATFORM.md` for specs.
...
- `res://addons/neocade_theme/scripts/neocade_theme.gd` — `@tool class_name NeoCadeTheme extends Theme` (the theme-engine script; concrete, instantiable, NOT abstract; users can subclass or instance it directly to author custom themes). Defines 11 `@export` properties total — top level (3): `style`, `raised`, `platform`; Style Overrides group (6, under `@export_group("Style Overrides")`): `source_color`, `corner_radius`, `spacing`, `raised_strength`, `focus_thickness`, `outline_width`; Advanced group (2): `use_runtime_popup_selection_icons`, `texture_cache` — and the `_regenerate_theme()` method that populates derived theme entry color/state values from the source-color role palette. Computes `var is_light: bool` from the generated surface role internally and branches formulas accordingly. Lives under `scripts/` with the reusable addon scripts.
...
- **Tech stack**: Godot 4.6+ Theme resource (.tres). Pure Theme/StyleBox primitives — no custom shaders, no GDExtension, no EditorPlugin / `plugin.cfg` in v1.
...
- Theme distributes as **1 canonical `.tres`** (`addons/neocade_theme/neocade_theme.tres`) + **1 theme-engine `.gd`** (`addons/neocade_theme/scripts/neocade_theme.gd` — `@tool class_name NeoCadeTheme extends Theme`, concrete and instantiable, 11 `@export` properties total) + **1 reusable UI control `.gd`** (`addons/neocade_theme/scripts/neocade_theme_option_button.gd` — `@tool class_name NeoCadeThemeOptionButton extends OptionButton`) + **1 optional autoload helper** (`addons/neocade_theme/scripts/neocade_theme_autoload.gd`). `NeoCadeTheme.Style` is the explicit direction selector for Bubble, Burst, Daybreak, Pulse, Slate, or Custom. No per-direction `.gd` files, no per-style `.tres` files, no `_dev/`, no `themes/`, no editor plugin, and no separate mobile resource. Dark/light handling is luminance-derived from the generated surface role.
...
| Showcase scene mirrors godot-demo-projects/gui/control_gallery scope | Phase 9 built the scorecard showcase; Phase 13 extended it to 10 sections with Role Variations while retaining the 37-row coverage strip. | ✓ Good |
| Theme controls in showcase: style/None + raised + platform | Phase 9 implemented null-theme comparison plus the style picker; current showcase also exposes runtime raised and platform controls for quick visual checks. | ✓ Good |
...
---
*Last updated: 2026-05-13 — synchronized with current implementation: canonical resource, 11 exports, source-color role palette, autoload helper, 10-section showcase, BINDING_TABLE=154, TYPE_VARIATIONS=66*
