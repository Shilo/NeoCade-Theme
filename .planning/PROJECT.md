...
- **Type variations**: the live `TYPE_VARIATIONS` registry contains 62 entries as of 2026-05-13: core runtime variations, 9 opt-in role variations, and editor/integration variations.
...
- [x] **Phase 12 — Signature Visual Moves: Defaults**: C4 HSV raised-depth tuning, C2' accent expansion, and C6 per-style signature moves are implemented. The 12-export public contract is preserved.
...
- Theme distributes as **1 canonical `.tres`** (`addons/neocade_theme/neocade_theme.tres`) + **1 theme-engine `.gd`** (`addons/neocade_theme/scripts/neocade_theme.gd` — `@tool class_name NeoCadeTheme extends Theme`, concrete and instantiable, 12 `@export` properties total) + **1 reusable UI control `.gd`** (`addons/neocade_theme/scripts/neocade_theme_option_button.gd` — `@tool class_name NeoCadeThemeOptionButton extends OptionButton`) + **1 optional autoload helper** (`addons/neocade_theme/scripts/neocade_theme_autoload.gd`). `NeoCadeTheme.Style` is the explicit direction selector for Bubble, Burst, Daybreak, Pulse, Slate, or Custom. No per-direction `.gd` files, no per-style `.tres` files, no `_dev/`, no `themes/`, no editor plugin, and no separate mobile resource. Dark/light handling is luminance-derived from `base_color`.
