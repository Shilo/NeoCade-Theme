...
- `res://addons/neocade_theme/_dev/neocade_theme.gd` — `@tool` `class_name NeoCadeTheme extends Theme` superclass (Phase 4 deliverable; carries chosen base direction's defaults baked in)
- `res://addons/neocade_theme/_dev/{name}_neocade_theme.gd` — per-direction subclasses (`class_name {Name}NeoCadeTheme extends NeoCadeTheme`); N-1 files where N = approved themes at Phase 3.4 gate
- `res://addons/neocade_theme/neocade_theme.tres` — saved instance of `NeoCadeTheme` superclass with chosen base direction's `@export` defaults (currently an empty scaffold; populated in Phase 4)
- `res://addons/neocade_theme/themes/{name}_neocade_theme.tres` — saved instances of personality subclass `.tres` files (N-1 files; `pop_plaza_neocade_theme.tres`, etc.)
- ~~`res://addons/neocade_theme/neocade_mobile_theme.tres`~~ — REMOVED per architecture revision; mobile is now a `@export platform=MOBILE` toggle on the superclass, not a separate file
...
- Theme distributes as `res://addons/neocade_theme/neocade_theme.tres` (the base) + N-1 subclass `.tres` files under `res://addons/neocade_theme/themes/` (per architecture revision 2026-05-04)
...
---
*Last updated: 2026-05-06 after Phase 3.3*
