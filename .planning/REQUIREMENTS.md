...
| FOUND-02 | Phase 4 (single concrete `NeoCadeTheme` class + 9-property export surface) | Plan 04-05 atomic commit `d9e405a` | Complete (Plan 04-05 atomic commit `d9e405a` shipped BINDING_TABLE 37-row freeze + CANONICAL_SLOT_NAMES freeze + 14-entry TYPE_VARIATIONS + `_resolve_recipe()` + iteration walk + default-font/font_size + per-variation set_font/set_font_size + per-Control icon wiring; `_regenerate_theme()` is feature-complete; loading any direction `.tres` in Plan 04-06/07 will populate all 37 scorecard Controls + 14 variations). |
