...
- [x] **SHOW-01**: `res://main.tscn` is the showcase scene; applied as project main scene. Uses NeoCade Theme as project theme (or per-scene `theme` override if leak avoidance preferred). Completed Phase 9.
...
- [x] **SHOW-04**: `NeoCadeThemeOptionButton` dropdown switches discovered `NeoCadeTheme` resources from `addons/neocade_theme/`, sorted alphabetically with optional `Default` first. Per 2026-05-08 correction, the showcase UI is editor-authored in `main.tscn`; only the dropdown uses `addons/neocade_theme/scripts/neocade_theme_option_button.gd`. Raised/platform variations remain `NeoCadeTheme` resource exports previewed through the inspector or consumer code, not showcase runtime controls.
