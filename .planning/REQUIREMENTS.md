...
- [x] **SHOW-04**: `NeoCadeThemeSwitcher` dropdown switches discovered `NeoCadeTheme` resources from `addons/neocade_theme/`, sorted alphabetically with optional `Default` first. Per 2026-05-08 correction, the showcase UI is editor-authored in `main.tscn`; only the dropdown uses `scripts/theme_switcher.gd`. Raised/platform variations remain `NeoCadeTheme` resource exports previewed through the inspector or consumer code, not showcase runtime controls.
