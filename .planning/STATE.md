...
**Core value:** A drop-in Godot 4.6 **flat MD3 / MD3 Expressive** Theme system at `res://addons/neocade_theme/` that ships one canonical `neocade_theme.tres` resource of type `NeoCadeTheme`, plus one concrete `addons/neocade_theme/scripts/neocade_theme.gd` class and one reusable `NeoCadeThemeOptionButton` picker. Consumers choose `preset` (Pulse, Slate, Bubble, Daybreak, Burst, or `NONE`) and toggle `raised` / `platform` / `base_color` / `accent_color` exports for flat/raised × desktop/mobile/AUTO variations — every built-in Control themed to a `godot-minimal-theme` bar of feature-completeness, accessible (WCAG 2.1 AA), universal across editor + runtime + all 6 Godot export targets. **No textures / no patterns / no embossing / no painterly chrome** (locked 2026-05-04 redirect). **Dynamic-theme architecture** feasibility-validated 2026-05-06 (Phase 3.2 strict gate 6/6 PASS in Godot 4.6.2), then consolidated 2026-05-08: single concrete `@tool class_name NeoCadeTheme extends Theme`, 10 exports including `preset`, luminance-derived `is_light`, no subclasses, no `_dev/`, no `themes/`, no per-preset `.tres`, no editor plugin, no `neocade_mobile_theme.tres`.
...
- ~~Roadmap (2026-05-04): 11-phase structure adopted from SUMMARY.md verbatim; mockup approval gate is hard blocker between Phase 3 and Phase 4; token-sharing strategy is `@tool` script generator (not `.tres` inheritance).~~ **SUPERSEDED 2026-05-04 by Phase 3 redirect + 2026-05-06e/f architecture simplification; updated 2026-05-08 by preset consolidation:** 15-phase structure (Phase 3 REDIRECTED, 3.1/3.2/3.3/3.4 inserted); mockup approval gate is between Phase 3.4 and Phase 4; token-sharing strategy is a single concrete `NeoCadeTheme` class + one canonical preset `.tres` resource, NOT a TokenSet generator script, NOT per-theme subclasses, and NOT per-preset resources.
...
- [Phase 06-lists-layout-range-tree-itemlist-tabs-containers-sliders-des]: Historical note: ResourceSaver canonicalization was explored for five direction resources, then superseded by the 2026-05-08 canonical resource + preset architecture.
...
- [Phase 07]: Historical note: the Phase 7 ResourceSaver helper was created but intentionally not run in Plan 07-01; this path was later superseded by the no-plugin canonical preset resource.
...
- [Phase 07-05]: Phase 7 full verification now treats the canonical 37 Control scorecard, GraphNode and GraphFrame extras, and preset-backed canonical resource invariant as closure gates. — Makes the final Phase 7 verifier match the plan's success criteria under the current no-plugin architecture.
...
**Track 4 — Flat addon layout + no root `.tres` (closed, 2026-05-06d; root-resource rule superseded 2026-05-08):** All `.gd` and `.tres` files live directly at `addons/neocade_theme/` — no `_dev/` or `themes/` subfolders (`fonts/` and `icons/` remain). Root `neocade_theme.tres` was removed during this historical track, then reintroduced by Track 6 as the single canonical preset resource. **The "5 subclass `.gd` files" portion of this track was further simplified by Track 5.**

**Track 5 — Single concrete class + data-driven `.tres` per direction (closed, 2026-05-06e; superseded by Track 6 on 2026-05-08):** Final 2026-05-06 simplification replaced subclass models with one concrete `NeoCadeTheme` class and five data-only direction `.tres` files. The class stayed correct, but the resource split was superseded by the later single-resource preset architecture.

**Track 6 — Canonical resource + explicit preset enum (current, 2026-05-08):** Five per-direction `.tres` files are consolidated into `addons/neocade_theme/neocade_theme.tres`. `NeoCadeTheme` keeps the single concrete `@tool class_name NeoCadeTheme extends Theme` model and adds a `preset` export (`NONE`, `PULSE`, `SLATE`, `BUBBLE`, `DAYBREAK`, `BURST`) for built-in directions. Public exports now total 10. `Preset.NONE` is the manual/custom mode; built-in presets apply the approved direction values and hidden direction personality. The reusable `NeoCadeThemeOptionButton` loads the canonical resource, duplicates it per selection, applies the selected preset, and emits `theme_selected(theme, index)`. The addon still has no editor plugin, no subclasses, no `_dev/`, no `themes/`, and no separate mobile resource.
