...
Note: older bullets in this section are historical session records. If they mention removed files such as per-style `.tres` resources or old registry counts, the current implementation snapshot above supersedes them.
...
- **Architecture revision (2026-05-04, simplified 2026-05-06e/f, consolidated 2026-05-08, Advanced exports added 2026-05-09, source-color rework 2026-05-13):** Replaced "4 static `.tres` per theme generated from TokenSet matrix" with dynamic export-driven Theme regeneration. Phase 3.2 first validated the superclass/subclass hypothesis, but production is now **single concrete `NeoCadeTheme` class + one canonical style `.tres` resource**. `NeoCadeTheme` is `@tool extends Theme` with 11 exports (`style`, `raised`, `platform`, `source_color`, `corner_radius`, `spacing`, `raised_strength`, `focus_thickness`, `outline_width`, `use_runtime_popup_selection_icons`, `texture_cache`). Setters dynamically regenerate theme entries from per-style source-color role palettes; `source_color` nudges authored role anchors instead of flattening every component into one tint. Per-direction personality lives behind the explicit `style` enum and direction style dictionaries, not per-direction `.gd` classes or per-style `.tres` files. `platform=AUTO` auto-detects via `OS.has_feature("mobile")` at runtime; `DESKTOP` and `MOBILE` are forced sizes. The old `base_color` / `accent_color` public API was intentionally removed without backwards compatibility. **NEW Phase 3.2 inserted** between Phase 3.1 and Phase 3.4 to validate this dynamic-theme feasibility.
...
- [Docs sync 2026-05-13]: Living `.planning/` docs reconciled against the current addon implementation. Current live registry counts are `BINDING_TABLE.size() == 154` and `TYPE_VARIATIONS.size() == 66`; the showcase has 10 sections and runtime `raised` / `platform` controls; the optional autoload helper is part of the shipped addon surface.
...
**Track 6 — Canonical resource + explicit style enum (current, 2026-05-08; Advanced exports added 2026-05-09; source-color rework 2026-05-13):** Five per-direction `.tres` files are consolidated into `addons/neocade_theme/neocade_theme.tres`. `NeoCadeTheme` keeps the single concrete `@tool class_name NeoCadeTheme extends Theme` model and adds a `style` export (`CUSTOM`, `PULSE`, `SLATE`, `BUBBLE`, `DAYBREAK`, `BURST`) for built-in directions. Public exports now total 11 after replacing the old `base_color` / `accent_color` API with `source_color` and keeping `use_runtime_popup_selection_icons` / `texture_cache`. `Style.CUSTOM` is the manual/custom mode; built-in styles apply the approved direction values and hidden direction personality. The reusable `NeoCadeThemeOptionButton` loads the canonical resource, duplicates it per selection, applies the selected style preset, and emits `theme_selected(theme, index)`. The addon still has no editor plugin, no subclasses, no `_dev/`, no `themes/`, and no separate mobile resource.
