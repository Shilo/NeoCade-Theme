...
last_activity: 2026-05-08 -- NeoCadeTheme editor save support added after ResourceSaver serialization regression
progress:
  total_phases: 15
  completed_phases: 14
  total_plans: 67
...
Last activity: 2026-05-08 -- ResourceSaver editor save regression fixed with plugin-registered NeoCadeTheme saver
...
- **Editor save support (2026-05-08):** Godot's built-in Theme saver serializes generated Theme entries when exports are changed and resources are saved. NeoCade now ships `plugin.cfg`, `scripts/neocade_theme_plugin.gd`, and `scripts/neocade_theme_resource_format_saver.gd`; the plugin registers/removes a custom saver with the editor lifecycle so direction resources remain data-only. The saver derives export names from `NeoCadeTheme.get_property_list()` usage flags instead of a hardcoded list.
