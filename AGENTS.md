...
- Final design tokens and mobile spec: `.planning/DESIGN_TOKENS.md`, `MOBILE-DESIGN-SPEC.md`
...
Before any GSD command, read `.planning/STATE.md`. If a requested step is skipped, refuse and name the missing step.

Per phase, commands run in order:

1. `/gsd-discuss-phase N`
2. `/gsd-plan-review-convergence N --opencode`
3. `/gsd-execute-phase N`
4. `/gsd-verify-work`
5. `/clear` only at the phase boundary

Do not run plain `/gsd-plan-phase` or plain `/gsd-review`; both are wrapped by `/gsd-plan-review-convergence`. Cross-AI peer review is mandatory for every phase through the convergence wrapper.

Project-specific `/clear` rule: clear only after `/gsd-verify-work` for phase N and before `/gsd-discuss-phase N+1`. Never clear between discuss, convergence planning, execution, and verification within the same phase.

Common skip refusals:

- Missing `DISCUSS.md`: "Run `/gsd-discuss-phase N` first."
- Missing or unconverged plan: "Run `/gsd-plan-review-convergence N --opencode` first."
- Advancing before verification: "Phase N not verified. Run `/gsd-verify-work` before advancing."
- Starting a new phase without boundary clear: "Type `/clear` first."

Hard-consequence verify-skip refusals:

- Phase 3.4 -> 4: mockup approval gate
- Phase 7 -> 8: 37/37 Control coverage before mobile duplication
- Phase 10 -> 11: ship-safety before publishing/release prep

Recovery and special commands: `/gsd-resume-work`, `/gsd-progress`, `/gsd-pause-work`, `/gsd-undo`, `/gsd-audit-uat`, `/gsd-ship`, and `$gsd-complete-milestone` after manual release/UAT.

Refuse parallel `/gsd-*` commands because they can corrupt shared state. Do not hand-edit `.planning/*` mid-execution unless the active GSD workflow explicitly calls for it.
