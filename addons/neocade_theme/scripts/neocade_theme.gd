...
static var _resource_format_saver: ResourceFormatSaver
static var _resource_format_saver_registered := false
static var _resource_format_saver_passthrough := false

func _init() -> void:
	_register_resource_format_saver()
	_regenerate_theme()


static func _register_resource_format_saver() -> void:
	if _resource_format_saver_registered:
		return

	_resource_format_saver = NeoCadeThemeResourceFormatSaver.new()
	ResourceSaver.add_resource_format_saver(_resource_format_saver, true)
	_resource_format_saver_registered = true
...
## Keeps NeoCadeTheme `.tres` files data-first: Godot's built-in Theme saver writes every
## generated stylebox/color/icon entry, so this saver round-trips through the built-in text
## saver and strips only formula-owned entries before the file hits disk.
class NeoCadeThemeResourceFormatSaver extends ResourceFormatSaver:
	const SCRIPT_EXT_RESOURCE_ID := "1_neocade_theme"
	const SCRIPT_FALLBACK_PATH := "res://addons/neocade_theme/scripts/neocade_theme.gd"
	const EXPORT_PROPERTY_NAMES := [
		"base_color",
		"accent_color",
		"raised",
		"platform",
	...
	func _recognize(resource: Resource) -> bool:
		return not NeoCadeTheme._resource_format_saver_passthrough and resource is NeoCadeTheme
	...
	func _save(resource: Resource, path: String, _flags: int) -> Error:
		...
		var result := _build_resource_text(theme, path)
	...
	func _set_uid(path: String, uid: int) -> Error:
		...
		if lines.is_empty() or not String(lines[0]).begins_with("[gd_resource"):
			return ERR_PARSE_ERROR

		lines[0] = _with_header_uid(String(lines[0]), ResourceUID.id_to_text(uid))
		var file := FileAccess.open(path, FileAccess.WRITE)
		if file == null:
			return ERR_CANT_OPEN
	...
	func _build_resource_text(theme: NeoCadeTheme, path: String) -> Dictionary:
		var temp_path := _temporary_save_path(path)
		_remove_file(temp_path)

		NeoCadeTheme._resource_format_saver_passthrough = true
		var save_error := ResourceSaver.save(theme, temp_path)
		NeoCadeTheme._resource_format_saver_passthrough = false
		if save_error != OK:
			_remove_file(temp_path)
	...
	func _strip_generated_resource_text(theme: NeoCadeTheme, path: String, serialized_text: String) -> String:
		...
		var generated_properties := _generated_theme_property_names(theme)
		...
		for raw_line in parsed["resource_lines"]:
			...
			if property_name.is_empty() or EXPORT_PROPERTY_NAMES.has(property_name):
				continue
		...
		var script_resource_id := _script_ext_resource_id(path)
		var lines: PackedStringArray = PackedStringArray()
		lines.append(_resource_header(path))
		lines.append("[ext_resource type=\"Script\" path=\"%s\" id=\"%s\"]" % [
			_escape_string(_theme_script_path(theme)),
		...
		for line in kept_resource_lines:
			lines.append(line)
		for property_name in _export_property_order(path):
	...
	func _temporary_save_path(path: String) -> String:
		return path.get_base_dir().path_join(path.get_file().get_basename() + ".neocade_tmp.tres")
	...
	func _generated_theme_property_names(theme: NeoCadeTheme) -> Dictionary:
		var generated := NeoCadeTheme.new()
		generated.base_color = theme.base_color
		generated.accent_color = theme.accent_color
		generated.raised = theme.raised
		generated.platform = theme.platform
		generated.corner_radius = theme.corner_radius
		generated.spacing = theme.spacing
		generated.raised_strength = theme.raised_strength
		generated.focus_thickness = theme.focus_thickness
		generated.outline_width = theme.outline_width
	...
	func _resource_header(path: String) -> String:
	...
	func _existing_uid_text(path: String) -> String:
		...
		return header.substr(uid_start, uid_end - uid_start)
	...
	func _with_header_uid(header: String, uid_text: String) -> String:
		...
		var uid_start := header.find(uid_marker)
		if uid_start == -1:
			return header.substr(0, header.length() - 1) + " uid=\"%s\"]" % _escape_string(uid_text)
	...
	func _script_ext_resource_id(path: String) -> String:
		var file := FileAccess.open(path, FileAccess.READ)
		if file == null:
			return SCRIPT_EXT_RESOURCE_ID
		...
		while not file.eof_reached():
			...
			if line.begins_with("[ext_resource") and line.find("type=\"Script\"") != -1:
				var parsed_id := _quoted_attribute(line, "id")
				file.close()
				return parsed_id if not parsed_id.is_empty() else SCRIPT_EXT_RESOURCE_ID

		file.close()
		return SCRIPT_EXT_RESOURCE_ID


	func _export_property_order(path: String) -> Array[String]:
		...
		if file != null:
			...
			while not file.eof_reached():
				...
				if EXPORT_PROPERTY_NAMES.has(property_name) and not seen.has(property_name):
