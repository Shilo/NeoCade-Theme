...
- Signals/Groups/class subsection rows use `Editor.prop_subsection_stylebox`. If the header should reveal the parent view edge, prefer tiny left/right content inset and no painted border; if it needs child-aligned rails, author only left/right borders there or in the `_left`/`_right` variants. Avoid full outlines that turn subsection rows into boxes.
...
- Godot's editor theme settings (`interface/theme/base_color`, `accent_color`, etc.) can leak through unstyled editor slots as obvious white/red fallback colors when the user changes those settings. If NeoCade is not intentionally adopting editor settings as its source of truth, add explicit `Editor` color slots for base/accent/font/icon/highlight/selection/error-warning-success/separator/axis colors so internal editor widgets stop falling back to Godot's editor config.
