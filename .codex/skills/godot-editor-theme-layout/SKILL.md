...
- Keep showcase/sample scenes referencing `res://addons/neocade_theme/neocade_theme.tres` externally. If a scene save embeds generated Theme entries, the text scene can balloon by hundreds of KiB and trigger Godot's large text resource warning; run the embedded-theme cleanup probe and verify the scene still opens.
- Cache static SVG loads and deterministic generated textures across regenerations. Rebuilding hue ramps, split grabbers, slider knobs, PopupMenu check/radio icons, and empty textures on every setter call can dominate regeneration time even when the stylebox table itself is modest.
