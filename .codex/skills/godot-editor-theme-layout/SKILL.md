...
- Signals/Groups/class subsection rows use `Editor.prop_subsection_stylebox`. If the header needs child-aligned side rails, author only left/right borders there or in the `_left`/`_right` variants; avoid full outlines that turn subsection rows into boxes.
