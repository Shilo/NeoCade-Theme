...
& 'C:\Programming_Files\Godot\Godot_v4.6.2-stable_win64.exe\Godot_v4.6.2-stable_win64.exe' --headless --editor --path . --script .planning/qa/theme-rescue/theme_editor_red_leak_probe.gd
...
When investigating editor-setting color leaks, ask the user to temporarily set obvious sentinel colors in the Godot editor first; do not assume the editor is already using sentinel values. For red leak testing, ask them to set `interface/theme/base_color` and `interface/theme/accent_color` to `#ff0000`, reproduce/reload the editor theme, then run `theme_editor_red_leak_probe.gd`. The sentinel color is only a temporary diagnostic aid and should not be treated as the normal theme configuration. Godot builds a generated editor theme from `interface/theme/*` and then merges the custom theme over it, so unowned editor slots can leak sentinel colors even when runtime controls are fully styled. Treat probe hits as missing theme ownership for real editor type/slot names, not as a reason to globally copy editor settings. `theme_editor_red_leak_probe.gd` scans both the merged `EditorTheme` type/slot table and the live editor `Control` tree for resolved red colors/styleboxes.
...
- Red sentinel leaks in editor screenshots often map to `EditorStyles` and editor-only variations, not ordinary runtime controls. `EditorStyles.Content` paints the main editor content panel, `EditorStyles.DebuggerPanel` paints the debugger bottom panel, `EditorStyles.PanelForeground` / `PanelForeground.panel` / `EditorInspectorForeground.panel` paint foreground editor panels, and `TabContainerInner` / `TabBarInner` own inner editor tab strips. Source-trace these first before changing base `Panel`, `TabContainer`, `TabBar`, or `MarginContainer`.
